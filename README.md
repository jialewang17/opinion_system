...
- **声量分析** (volume): 各渠道声量统计
- **主题分析** (topic): BERTopic+Qwen的智能主题聚类和命名
- **流体动力学分析** (fluid): 舆论场五维状态面板和热度预测
...
- **主题建模**: BERTopic, UMAP, HDBSCAN
- **向量化**: 通义千问 text-embedding-v4
- **向量数据库**: LanceDB
- **关系数据库**: PostgreSQL
...
# 如果遇到依赖安装问题，可以单独安装关键依赖：
# pip install dashscope>=1.14.0  # 阿里云通义千问SDK（必需）
```
...
**注意**: 
- 主题分析（TopicBertopic）是独立功能，与数据分析流水线平行
- 两者都基于数据库数据（都需要先Fetch从数据库提取数据）
- 数据分析流水线：Fetch → Analyze（基于fetch数据）
- 主题分析：Fetch → TopicBertopic（基于fetch数据）
- 两者数据来源相同（都是数据库），处理流程不同，是并行关系
...
**数据来源**: 
- Analyze模块基于 `data/fetch/` 目录的数据进行分析
- 这些数据是通过Fetch命令从数据库提取的
- 数据包括：`总体.csv` 和各渠道的 `*.csv` 文件
...
**注意**: 运行Analyze前需要先运行Fetch命令从数据库提取数据。
...
#### 3. 主题分析（TopicBertopic）

基于BERTopic+Qwen的主题聚类分析，自动识别和合并相似主题。

**说明**: 
- **独立功能**：与数据分析流水线平行，是独立的分析功能
- **数据来源**：从数据库提取的数据（需要先运行Fetch从数据库提取数据到`data/fetch/{专题}/{时间范围}/`）
- **处理流程**：Fetch从数据库提取 → TopicBertopic分析（使用BERTopic进行主题建模 → 使用大模型进行主题再聚类和命名）
- **输出位置**：`data/topic/{专题}/{时间范围}/`

**与数据分析流水线的区别**：
- **数据分析流水线**：Fetch从数据库提取 → Analyze分析（7大维度分析）
- **主题分析**：Fetch从数据库提取 → TopicBertopic分析（主题聚类分析）
- **两者关系**：数据来源相同（都是数据库），处理流程不同，是并行关系，可以同时运行
...
- `--end`: 结束日期（可选，格式：YYYY-MM-DD）
  - 如果提供 `--end`：分析日期范围的数据，路径格式为 `{start}_{end}`
  - 如果不提供 `--end`：使用 `start` 作为单日期，路径格式为 `{start}`

**日期范围分析**:
```bash
# 先运行Fetch从数据库提取数据（如果还没有fetch数据）
python main.py Fetch --topic 控烟 --start 2024-01-01 --end 2024-01-15

# 然后运行主题分析（基于fetch数据）
python main.py TopicBertopic --topic 控烟 --start 2024-01-01 --end 2024-01-15
# 读取：data/fetch/控烟/2024-01-01_2024-01-15/
# 输出：data/topic/控烟/2024-01-01_2024-01-15/
```

**单日期分析**:
```bash
# 先运行Fetch从数据库提取数据（如果还没有fetch数据）
python main.py Fetch --topic 控烟 --start 2024-01-01 --end 2024-01-01

# 然后运行主题分析（不提供--end参数，使用start作为单日期）
python main.py TopicBertopic --topic 控烟 --start 2024-01-01
# 读取：data/fetch/控烟/2024-01-01/
# 输出：data/topic/控烟/2024-01-01/
```

**注意**: 运行TopicBertopic前需要先运行Fetch命令从数据库提取数据。如果使用单日期分析，Fetch命令也需要使用相同的日期范围（`--start` 和 `--end` 相同）。

**自定义参数**:
```bash
# 使用自定义用户词典
python main.py TopicBertopic --topic 控烟 --start 2024-01-01 --end 2024-01-15 --userdict "configs/userdict.txt"

# 使用自定义停用词
python main.py TopicBertopic --topic 控烟 --start 2024-01-01 --end 2024-01-15 --stopwords "configs/stopwords.txt"
```

**输出结果**:
```
data/topic/{专题}/{时间范围}/
├── 1主题统计结果.json          # 主题文档统计、关键词、2D坐标
├── 2主题关键词.json             # 各主题关键词
├── 3文档2D坐标.json             # 文档在2D空间的坐标
├── 4大模型再聚类结果.json        # 大模型合并后的主题
└── 5重聚类主题关键词.json        # 合并后的主题关键词
```

#### 4. 流体动力学分析（FluidAnalysis）

基于舆论流体力学模型的五维状态面板分析和热度预测。

**说明**: 
- **独立功能**：与数据分析流水线平行，是独立的分析功能
- **数据来源**：从数据库提取的数据（需要先运行Fetch从数据库提取数据到`data/fetch/{专题}/{时间范围}/`）
- **处理流程**：Fetch从数据库提取 → FluidAnalysis分析（计算五维状态面板、综合指标、热度预测）
- **输出位置**：`data/fluid/{专题}/{时间范围}/`

**与数据分析流水线的区别**：
- **数据分析流水线**：Fetch从数据库提取 → Analyze分析（7大维度分析）
- **流体动力学分析**：Fetch从数据库提取 → FluidAnalysis分析（舆论场状态和热度预测）
- **两者关系**：数据来源相同（都是数据库），处理流程不同，是并行关系，可以同时运行

**功能特性**：
- **五维状态面板**：流速(I)、粘度(C)、密度(D)、温度(T)、压力(S)
- **综合指标**：涡度(Ω)、压强梯度(G)、雷诺数(Re)、热度(H)
- **热度预测**：基于舆论流体力学模型的热度变化预测
- **临界点分析**：识别舆论场的临界状态
- **时间窗口分析**：支持自定义时间窗口（默认3小时）
...
- `--end`: 结束日期（可选，格式：YYYY-MM-DD）
  - 如果提供 `--end`：分析日期范围的数据，路径格式为 `{start}_{end}`
  - 如果不提供 `--end`：使用 `start` 作为单日期，路径格式为 `{start}`
- `--window-hours`: 时间窗口大小（小时），默认3小时
- `--file`: 可选，指定要分析的文件名（例如: "论坛.csv"），只分析该文件
...
# 然后运行流体动力学分析（基于fetch数据）
python main.py FluidAnalysis --topic 控烟 --start 2024-01-01 --end 2024-01-15
# 读取：data/fetch/控烟/2024-01-01_2024-01-15/
# 输出：data/fluid/控烟/2024-01-01_2024-01-15/
```
...
# 然后运行流体动力学分析（不提供--end参数，使用start作为单日期）
python main.py FluidAnalysis --topic 控烟 --start 2024-01-01
# 读取：data/fetch/控烟/2024-01-01/
# 输出：data/fluid/控烟/2024-01-01/
```

**自定义时间窗口**:
```bash
# 使用6小时时间窗口
python main.py FluidAnalysis --topic 控烟 --start 2024-01-01 --end 2024-01-15 --window-hours 6
```

**指定分析特定文件**:
```bash
# 只分析论坛渠道的数据
python main.py FluidAnalysis --topic 控烟 --start 2024-01-01 --end 2024-01-15 --file 论坛.csv
```

**注意**: 运行FluidAnalysis前需要先运行Fetch命令从数据库提取数据。如果使用单日期分析，Fetch命令也需要使用相同的日期范围（`--start` 和 `--end` 相同）。

**输出结果**:
```
data/fluid/{专题}/{时间范围}/
├── indicators_unified.json       # 汇总JSON结果（基于全部文件）
├── 论坛_indicators.json          # 按单个文件输出的JSON结果示例
├── 视频_indicators.json
├── 微博_indicators.json
├── 微信_indicators.json
├── 新闻_indicators.json
├── 自媒体号_indicators.json
└── 总体_indicators.json

# 每个 *_indicators.json 的结构包含：
- 舆论场五维状态面板    # 流速、粘度、密度、温度、压力
- 综合指标可视化        # 涡度、压强梯度、雷诺数
- 热度计算              # 热度值
- 概览                  # 总帖子数、时间跨度、用户数等
- heatPrediction        # 热度预测数据
- criticalPoint         # 临界点分析
- analysis              # 分析和建议
- metrics               # 指标阈值和状态
- 计算结果              # 各时间窗口的详细结果
...
**五维状态面板**:
- **流速(I)**: 信息扩散速度，反映信息传播效率（标准化值，0-1）
- **粘度(C)**: 平台信息集中度，衡量信息是否破圈（标准化值，0-1）
- **密度(D)**: 舆情参与密度，用户参与程度（0-1）
- **温度(T)**: 舆情温度，整体情感倾向（0-1）
- **压力(S)**: 外部压力，社会敏感性与外部干预强度（标准化值，0-1）

**综合指标**:
- **涡度(Ω)**: 舆论极化程度，展示观点集群与对立情况
- **压强梯度(G)**: 影响力驱动，展示意见领袖与普通用户间的态度差异驱动
- **雷诺数(Re)**: 舆论场稳定度，区分有序传播（层流）、过渡态、混乱态（湍流）
- **热度(H)**: 基于舆论流体力学模型计算的累计热度
...
```json
{
  "舆论场五维状态面板": {
    ...
    "压力": {
      "值": 0.38,
      "说明": "外部压力，社会敏感性与外部干预强度"
    }
  },
  "综合指标可视化": {
    "涡度": {
      "值": 0.45,
      "说明": "舆论极化程度，展示观点集群与对立情况"
    },
    "压强梯度": {
      "值": 0.32,
      "说明": "影响力驱动，展示意见领袖与普通用户间的态度差异驱动"
    },
    "雷诺数": {
      "值": 1250,
      "流态": "过渡",
      "说明": "舆论场稳定度，区分有序传播（层流）、过渡态、混乱态（湍流）"
    }
  },
  "热度计算": {
    "热度": {
      "值": 4.25,
      "说明": "基于舆论流体力学模型计算的累计热度"
    }
  }
}
```
...
1. **提示词配置** (`configs/prompt/topic_bertopic/recluster.yaml`):
   
   配置大模型再聚类使用的提示词，用于控制主题合并和命名的规则。详见[配置说明](#配置说明)部分的"主题分析配置"。

2. **用户词典配置** (`configs/userdict.txt`):
   
   配置自定义词典，用于改进分词效果。每行一个词，支持领域专业术语。
   
   **使用说明**:
   - 每行一个词，不需要标点符号
   - 添加专业术语、领域词汇、新词等
   - 词典中的词会被Jieba优先识别为一个完整词
   - 如果不指定 `--userdict` 参数，默认使用 `configs/userdict.txt`

3. **停用词配置** (`configs/stopwords.txt`):
   
   配置停用词列表，用于过滤无意义的词语。每行一个词。
   
   **使用说明**:
   - 每行一个停用词
   - 常见的停用词包括：的、了、在、是、等
   - 如果不指定 `--stopwords` 参数，默认使用 `configs/stopwords.txt`
...
│   ├── llm.yaml              # LLM模型配置
│   ├── stopwords.txt         # 停用词表
│   ├── userdict.txt          # 用户词典（可选）
│   └── prompt/               # 提示词模板
│       ├── explain/           # 解读提示词
│       ├── filter/           # 筛选提示词
│       ├── router_vec/       # 向量化提示词
│       ├── router_retrieve/  # 检索提示词
│       └── topic_bertopic/   # 主题分析提示词
├── data/                      # 数据目录
│   ├── raw/                  # 原始数据
│   ├── merge/                # 合并数据
│   ├── clean/                # 清洗数据
...
│   ├── topic/                # 主题分析模块
│   │   ├── __init__.py       # 模块导出
│   │   └── data_bertopic_qwen.py  # BERTopic+Qwen主题分析实现
│   ├── fluid/                # 流体动力学分析模块
│   │   ├── __init__.py       # 模块导出
│   │   └── fluid_analysis.py # 流体动力学分析实现
│   └── utils/                # 工具模块
│       ├── ai/               # AI接口
│       ├── io/               # 数据IO
│       ├── logging/          # 日志系统
...
### 5. 主题分析配置 (`configs/prompt/topic_bertopic/recluster.yaml`)

配置大模型再聚类使用的提示词，用于控制主题合并和命名的规则。

```yaml
# BERTopic大模型再聚类提示词配置
prompts:
  topic_bertopic_recluster:
    system: |
      你是一个专业的控烟领域主题分析专家，擅长将相似的控烟相关主题进行归纳和合并，并对其进行命名。
    user: |
      分析以下控烟相关主题，将语义相似的主题合并为{TARGET_TOPICS}个左右的主题。
      
      【控烟研究框架维度】(参考)
      1.烟草与健康相关议题：疾病关联、病理机制、流行病学、戒烟效益
      2.无烟立法相关议题：法律制定、法规修订、实施情况
      3.控烟公约相关议题：FCTC进程、履约评估、国际协商、执行情况
      4.无烟场所相关议题：公共场所禁烟、禁烟范围、吸烟区设置、执法效果
      5.监管体制与处罚相关议题：执法机构、职责划分、监督机制、违法处罚
      6.包装警示相关议题：健康警示、图形标识、包装轮换、禁止误导性标签
      7.税收价格相关议题：税费政策、价格调控、最低定价、非法贸易影响
      8.影视作品及广告相关议题：广告限制、影视控烟、广告植入、间接促销
      9.公共议题、抗争性议题与公共事件相关议题：社会运动、公众抗议、重大事件、舆论讨论
      10.公共卫生（PM2.5等）相关议题：二手烟与PM2.5、环境污染、健康中国战略
      11.文化价值相关议题：社会文化、习俗反思（敬烟）、礼品文化、无烟风尚
      12.青少年相关议题：禁止向青少年售烟、校园周边监管、吸烟率调查、流行与干预
      13.港澳台及国际经验相关议题：港澳台经验、国际经验、政策比较、立法实践
      14.电子烟相关议题：新型烟草监管、健康风险、口味禁令、税收政策
      输入数据：
      {input_data}
      
      任务要求：
      1. 将语义相似的主题合并
      2. 合并后主题数控制在{TARGET_TOPICS}个左右（±2个）
      3. 保持主题间区分度
      4. 为每个合并后的主题生成一个符合控烟研究框架、完整、精准、具有学术概括性的中文主题名称（6-8个字，必须包含完整的语义结构）
      
      【合并原则】
      1. 内容相似：关键词重叠度高
      2. 框架一致：属于同一研究维度
      3. 避免过度合并：差异明显的主题保持独立
      4. 主题命名：基于主题描述，提炼核心概念，符合控烟研究框架
      
      【主题命名要求】
      - 名称长度为 6-8个字，确保完整性和描述性
      - 符合控烟研究框架，具有学术概括性
      - 基于主题描述提炼核心概念
      - 命名必须包含完整的语义结构，不能只是单一词汇
        * 正确示例："青少年烟草防护"、"烟草税政监管"、"公共场所控烟执法"、"戒烟干预方法"
        * 错误示例："青少年"、"烟草"、"执法"、"干预"（过于简洁）
        ...
        "合并方案": [
          {
            ...
            "原始主题集合": ["主题3", "主题5", "主题9"],
            "主题描述": "这个主题主要关注...",
            "主题命名": "完整的主题中文名称（6-8个字，必须包含完整的语义结构）",
            "合并理由": "这些主题都涉及...",
            "合并后关键词": ["关键词1", "关键词2", "关键词3", "关键词4", "关键词5", "关键词6", "关键词7", "关键词8", "关键词9", "关键词10"]
          }
        ],
        "分析说明": "整体聚类策略说明"
      }
...
**提示词变量**:
- `{TARGET_TOPICS}`: 目标主题数量（自动替换，默认8个）
- `{input_data}`: 主题信息数据（自动替换）

**自定义提示词**:
- 根据不同的专题和研究框架，修改提示词中的研究框架维度
- 调整主题命名要求和合并原则
- 修改输出格式以适应不同的分析需求
