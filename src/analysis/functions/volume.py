...
def analyze_volume_overall(df: pd.DataFrame, topic: str, date: str, logger=None) -> Dict[str, Any]:
    """
    ...
    try:
        # 从warehouse目录读取各渠道CSV文件进行统计
        
        warehouse_dir = bucket("warehouse", topic, date)
        if not warehouse_dir.exists():
            log_error(logger, f"warehouse目录不存在: {warehouse_dir}", "Analyze")
            return {"data": []}
        
        # 获取所有CSV文件（排除总体.csv）
        csv_files = [f for f in warehouse_dir.glob('*.csv') if f.name != '总体.csv']
        if not csv_files:
            log_error(logger, f"warehouse目录中没有找到渠道CSV文件: {warehouse_dir}", "Analyze")
            return {"data": []}
...
def analyze_volume_by_channel(df: pd.DataFrame, channel_name: str, topic: str, date: str, logger=None) -> Dict[str, Any]:
    """
    ...
    try:
        # 从warehouse目录读取对应渠道的CSV文件
        
        warehouse_dir = bucket("warehouse", topic, date)
        csv_file = warehouse_dir / f"{channel_name}.csv"
