...
def analyze_geography_overall(df: pd.DataFrame, logger=None) -> Dict[str, Any]:
    ...
    try:
        ...
        log_success(logger, "geography | 总体 分析完成", "geography")
        return result

    except Exception as e:
        log_error(logger, f"geography | 总体 分析失败: {e}", "geography")
        return {}
...
def analyze_geography_by_channel(df: pd.DataFrame, channel_name: str, logger=None) -> Dict[str, Any]:
    ...
    try:
        ...
        log_success(logger, f"geography | {channel_name} 分析完成", "geography")
        return result
    except Exception as e:
        log_error(logger, f"geography | {channel_name} 分析失败: {e}", "geography")
        return {}
