...
from ...utils.logging import log_success, log_error, log_skip
...
def analyze_classification_overall(df: pd.DataFrame, logger) -> Dict[str, Any]:
    ...
    try:
        if df.empty:
            log_skip(logger, "总体数据为空，跳过分类分析", "Classification")
            return {}
        
        # 检查classification字段是否存在
        if 'classification' not in df.columns:
            log_error(logger, "数据中缺少classification字段", "Classification")
            return {}
        ...
        log_error(logger, f"总体分类分析失败: {e}", "classification")
        return {}
...
def analyze_classification_by_channel(df: pd.DataFrame, channel_name: str, logger) -> Dict[str, Any]:
    ...
    try:
        if df.empty:
            log_skip(logger, f"渠道 {channel_name} 数据为空，跳过分类分析", "Classification")
            return {}
        
        # 检查classification字段是否存在
        if 'classification' not in df.columns:
            log_error(logger, f"渠道 {channel_name} 数据中缺少classification字段", "Classification")
            return {}
        ...
        log_error(logger, f"渠道分类分析失败: {e}", "classification")
        return {}
