...
from .io.databases import fetch_range, upload_filtered_excels
...
def data_pipeline(topic, date):
    ...
    """
    logger = setup_logger(topic, date)
    log_module_start(logger, "DataPipeline")
    ...
    try:
        ...
        log_success(logger, "数据清洗和存储流水线执行完成", "DataPipeline")
        return True
        
    except Exception as e:
        log_error(logger, f"数据流水线执行异常: {e}", "DataPipeline")
        return False
...
def analysis_pipeline(topic, start, end):
    ...
    """
    logger = setup_logger(topic, start)
    log_module_start(logger, "AnalyzePipeline")
    
    try:
        # 1. 提数
        log_module_start(logger, "Fetch")
        if not fetch_range(topic, start, end, start, logger):
        ...
        # 使用时间范围格式的目录名
        date_range = f"{start}_{end}"
        if not run_Analyze(topic, date_range, logger):
            log_error(logger, "模块执行失败", "Analyze")
            return False
        
        log_success(logger, "数据分析流水线执行完成", "AnalyzePipeline")
        return True
        
    except Exception as e:
        log_error(logger, f"分析流水线执行异常: {e}", "AnalyzePipeline")
        return False
